...
int main(int argc, char** argv)
{
  ...
  ros::ServiceClient stackDescriptionServ = nh.serviceClient<pal_wbc_msgs::GetStackDescription>(
      "/whole_body_kinematic_controller/get_stack_description");
  ros::ServiceClient popTaskServ = nh.serviceClient<pal_wbc_msgs::PopTask>(
      "/whole_body_kinematic_controller/pop_task");
  ros::ServiceClient pushTaskServ = nh.serviceClient<pal_wbc_msgs::PushTask>(
      "/whole_body_kinematic_controller/push_task");
  ros::ServiceClient pushPopTaskServ = nh.serviceClient<pal_wbc_msgs::PushPopTask>(
      "/whole_body_kinematic_controller/push_pop_task");
  ros::ServiceClient getTaskErrorServ = nh.serviceClient<pal_wbc_msgs::GetTaskError>(
...

  {
    ...
    ROS_INFO_STREAM("Task: " << srv.request.id
                             << " error norm is: " << srv.response.taskError.error_norm);
    int cont = 0;
...

  {
    ...
    property_bag::PropertyBag taskProperties(
        "taskType", std::string("pal_wbc/GazePointKinematicMetaTask"));
